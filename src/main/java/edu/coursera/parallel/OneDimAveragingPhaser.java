...
public final class OneDimAveragingPhaser {
    ...
    public static void main(String[] args) {
        final int n = 1000000; // Array size
        final int iterations = 100; // Number of iterations to run
        final double[] myVal = new double[n + 2];
        final double[] myNew = new double[n + 2];

        // Initialize your arrays with some data
        ...
        // Warm-up phase
        for (int i = 0; i < 5; i++) {
            runSequential(iterations, myNew.clone(), myVal.clone(), n);
        }
        ...
        System.out.println("Sequential time: " + (endTimeSeq - startTimeSeq) + " nanoseconds");

        // Parallel measurement
        long startTimePar = System.nanoTime();
        runParallelBarrier(iterations, myNew.clone(), myVal.clone(), n, Runtime.getRuntime().availableProcessors());
        long endTimePar = System.nanoTime();

        // Report parallel time
        System.out.println("Parallel time: " + (endTimePar - startTimePar) + " nanoseconds");

        long startTimeFuzPar = System.nanoTime();
        runParallelFuzzyBarrier(iterations, myNew.clone(), myVal.clone(), n, Runtime.getRuntime().availableProcessors());
        long endTimeFuzPar = System.nanoTime();

        System.out.println("Parallel Fuzzer time: " + (endTimeFuzPar - startTimeFuzPar) + " nanoseconds");
