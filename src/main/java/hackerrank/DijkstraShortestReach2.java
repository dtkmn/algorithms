    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<T; i++) {
                ...
                if(visited[num] == 0) {
                    ...
                    if (edgeList != null) {
                        for (Integer num1 : edgeList.keySet()) {
                            ...
                            if (currentWeight == -1 || dp[num] + edge.weight < currentWeight) {
                                dp[edge.to] = dp[num] + edge.weight;
                            }
                            if (visited[edge.to] == 0)
                                nodeQueue.add(new Node(edge.to, dp[edge.to]));
                        }
                    }
                    visited[num] = 1;
                }
            ...
            for(int m=1; m<dp.length; m++) {
                if(m!=start) System.out.print(dp[m] + " ");
            }
            System.out.println();
        }
